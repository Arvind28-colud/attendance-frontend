...
import { getStudentAttendance, getSubjectsByCourse, getAllTimetable, getAssignments } from '../api'
...
function FaceVerifyModal({ rollNo, storedFaceData, onSuccess, onClose }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: 28, maxWidth: 380, width: '100%', boxShadow: '0 20px 60px rgba(0,0,0,0.5)' }}>
        ...
        {!rollConfirmed && (
          <div>
            <p style={{ fontSize: 13, color: 'var(--text3)', marginBottom: 16 }}>First, confirm your identity by entering your roll number.</p>
            <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--text3)', display: 'block', marginBottom: 6 }}>Your Roll Number *</label>
            <input className="input-field" value={rollInput} onChange={e => setRollInput(e.target.value)} placeholder="Enter your roll number" style={{ marginBottom: 14, width: '100%' }} onKeyDown={e => e.key === 'Enter' && handleConfirmRoll()}/>
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={onClose} className="btn btn-ghost" style={{ flex: 1, justifyContent: 'center' }}>Cancel</button>
              <button onClick={handleConfirmRoll} className="btn btn-primary" disabled={!rollInput.trim()} style={{ flex: 1, justifyContent: 'center' }}>Confirm & Open Camera</button>
            </div>
          </div>
        )}
        ...
        {rollConfirmed && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }}>
            <div style={{ position: 'relative', width: 240, height: 280 }}>
              <video ref={videoRef} style={{ width: 240, height: 280, objectFit: 'cover', borderRadius: '50% / 45%', display: (status === 'matched' || status === 'failed' || status === 'loading') ? 'none' : 'block', transform: 'scaleX(-1)', background: '#111' }} muted playsInline/>
              {(status === 'loading' || status === 'verifying') && (
                <div style={{ width: 240, height: 280, borderRadius: '50% / 45%', background: '#111', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 10 }}>
                  <Loader size={32} color="#6c63ff" style={{ animation: 'spin 1s linear infinite' }}/>
                  <span style={{ fontSize: 12, color: '#aaa', textAlign: 'center', padding: '0 20px' }}>{status === 'verifying' ? 'Comparing faces...' : 'Loading...'}</span>
                </div>
              )}
              {status === 'matched' && (
                <div style={{ width: 240, height: 280, borderRadius: '50% / 45%', background: 'rgba(34,211,160,0.15)', border: '3px solid #22d3a0', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 10 }}>
                  <ShieldCheck size={48} color="#22d3a0"/><span style={{ fontSize: 13, color: '#22d3a0', fontWeight: 700 }}>Matched!</span>
                </div>
              )}
              {status === 'failed' && (
                <div style={{ width: 240, height: 280, borderRadius: '50% / 45%', background: 'rgba(255,77,106,0.15)', border: '3px solid #ff4d6a', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 10 }}>
                  <ShieldX size={48} color="#ff4d6a"/><span style={{ fontSize: 13, color: '#ff4d6a', fontWeight: 700 }}>Not Matched</span>
                </div>
              )}
              {(status === 'ready' || status === 'detected') && (
                <div style={{ position: 'absolute', inset: 0, borderRadius: '50% / 45%', border: `3px solid ${faceDetected ? '#22d3a0' : '#6c63ff'}`, boxShadow: `0 0 0 3px ${faceDetected ? 'rgba(34,211,160,0.3)' : 'rgba(108,99,255,0.2)'}`, pointerEvents: 'none', transition: 'border-color 0.3s' }}/>
              )}
              {status === 'ready' && faceDetected && (
                <div style={{ position: 'absolute', bottom: -12, left: '50%', transform: 'translateX(-50%)', background: '#22d3a0', color: '#000', fontSize: 10, fontWeight: 700, padding: '3px 10px', borderRadius: 99, whiteSpace: 'nowrap' }}>✓ Face detected</div>
              )}
            </div>
            <p style={{ fontSize: 13, textAlign: 'center', marginTop: 8, color: status === 'matched' ? 'var(--green)' : status === 'failed' ? 'var(--red)' : 'var(--text3)', fontWeight: status === 'matched' || status === 'failed' ? 700 : 400 }}>{message}</p>
            {status === 'ready' && (
              <button onClick={handleVerify} disabled={!faceDetected} className="btn btn-primary" style={{ width: '100%', justifyContent: 'center', opacity: faceDetected ? 1 : 0.5 }}>
            ...
            {status === 'failed' && (
              <div style={{ display: 'flex', gap: 10, width: '100%' }}>
                <button onClick={onClose} className="btn btn-ghost" style={{ flex: 1, justifyContent: 'center' }}>Cancel</button>
                <button onClick={handleRetry} className="btn btn-primary" style={{ flex: 1, justifyContent: 'center' }}><RefreshCw size={15}/> Retry</button>
              </div>
            )}
          </div>
        )}
...
export default function StudentDashboard({ showToast }) {
  ...
  const [studentDetailTab, setStudentDetailTab] = useState('attendance')
  ...
  // When a subject is selected, reset tabs and load assignments
  useEffect(() => {
    if (selectedSub) {
      ...
      // Load assignments
      setAssLoading(true)
      getAssignments(selectedSub)
        .then(res => setSubjectAssignments(res.data))
  ...
  const handleMarkAttendance = async (subjectId) => {
    ...
    try {
      ...
      await axios.post(`${BASE_URL}/attendance/mark`, {
        ...
        gps_lat: pos.coords.latitude, gps_lng: pos.coords.longitude, face_ok: true,
      })
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      {showFaceModal && storedFaceData && (
        <FaceVerifyModal rollNo={user.roll_no} storedFaceData={storedFaceData} onSuccess={handleFaceVerified} onClose={() => setShowFaceModal(false)}/>
      )}
      ...
      <header style={{ background: 'var(--surface)', borderBottom: '1px solid var(--border)', padding: '0 24px', height: 60, display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 100 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          {selectedSub && (
            <button onClick={() => { setSelectedSub(null); setGpsStatus(null); setFaceVerified(false) }} style={{ background: 'none', border: 'none', color: 'var(--text2)', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, marginRight: 4 }}>
              <ArrowLeft size={18}/>
            </button>
          )}
      ...
      {selectedSub && subjectDetail ? (
        <div className="fade-in" style={{ maxWidth: 700, margin: '0 auto', padding: '28px 20px' }}>
            ...
            {subjectSlots.length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {subjectSlots.map((slot, i) => {
                  ...
                  return (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', borderRadius: 'var(--radius2)', background: live ? 'rgba(34,211,160,0.1)' : 'var(--bg2)', border: `1px solid ${live ? 'var(--green)' : 'var(--border)'}` }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: 20, marginBottom: 20 }}>
            ...
            <div style={{ padding: 14, borderRadius: 'var(--radius2)', marginBottom: 12, background: faceVerified ? 'rgba(34,211,160,0.08)' : 'var(--bg2)', border: `1px solid ${faceVerified ? 'var(--green)' : 'var(--border)'}` }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <div style={{ width: 32, height: 32, borderRadius: '50%', background: faceVerified ? 'rgba(34,211,160,0.2)' : 'var(--bg)', border: `2px solid ${faceVerified ? 'var(--green)' : 'var(--border)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                ...
                {!faceVerified && (
                  <button onClick={handleOpenFaceVerify} disabled={!canMarkNow} className="btn btn-primary" style={{ fontSize: 12, padding: '7px 14px', opacity: canMarkNow ? 1 : 0.5 }}>
            ...
            {gpsStatus && (
              <div style={{ padding: 12, borderRadius: 'var(--radius2)', marginBottom: 12, background: gpsStatus.verified ? 'rgba(34,211,160,0.1)' : 'rgba(255,77,106,0.1)', border: `1px solid ${gpsStatus.verified ? 'var(--green)' : 'var(--red)'}`, fontSize: 13, color: gpsStatus.verified ? 'var(--green)' : 'var(--red)', display: 'flex', alignItems: 'center', gap: 8 }}>
                <MapPin size={14}/> {gpsStatus.message}
              </div>
            )}
            <div style={{ padding: 14, borderRadius: 'var(--radius2)', marginBottom: 12, background: 'var(--bg2)', border: 'var(--border)', opacity: faceVerified ? 1 : 0.5 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: faceVerified ? 12 : 0 }}>
                <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'var(--bg)', border: '2px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              {faceVerified && (
                <button onClick={() => handleMarkAttendance(selectedSub)} disabled={marking || !faceVerified} className="btn btn-success" style={{ width: '100%', justifyContent: 'center', padding: 12, fontSize: 14 }}>
                  {marking ? <><span className="spinner" style={{ borderTopColor: '#000' }}/> Verifying GPS...</> : <><MapPin size={15}/> Mark Present</>}
                </button>
              )}
            </div>
          ...
          {/* ── Detail Tabs: Attendance History + Lab Records ── */}
          <div style={{ display: 'flex', gap: 4, marginBottom: 16, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: 4 }}>
            {[{ id: 'attendance', label: '📋 Attendance' }, { id: 'lab', label: '🧪 Lab Records' }].map(t => (
              <button key={t.id} onClick={() => setStudentDetailTab(t.id)} style={{
                flex: 1, padding: '9px 16px', borderRadius: 8, fontSize: 13, fontWeight: 600,
                background: studentDetailTab === t.id ? 'var(--accent)' : 'transparent',
                color: studentDetailTab === t.id ? 'white' : 'var(--text3)', transition: 'all 0.2s',
          ...
          {studentDetailTab === 'attendance' && (
            <div className="fade-in" style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', overflow: 'hidden' }}>
          ...
          {studentDetailTab === 'lab' && (
            <div className="fade-in" style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', overflow: 'hidden' }}>
              ...
              {assLoading ? (
                ...
                <div className="empty-state" style={{ padding: 40 }}>No assignments posted yet</div>
                ...
                return (
                  <div key={i} style={{ padding: '16px 18px', borderBottom: i < subjectAssignments.length - 1 ? '1px solid var(--border)' : 'none' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: 12, color: 'var(--text3)' }}>
                          Posted: {new Date(a.created_at).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}
                        </div>
                      </div>
        ...
        <div style={{ maxWidth: 1000, margin: '0 auto', padding: '28px 20px' }}>
          ...
          <div style={{ display: 'flex', gap: 4, marginBottom: 24, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: 4 }}>
            {[{ id: 'overview', label: '📊 Subjects' }, { id: 'chart', label: '📈 Charts' }].map(t => (
              <button key={t.id} onClick={() => setActiveTab(t.id)} style={{ flex: 1, padding: '9px 16px', borderRadius: 8, fontSize: 13, fontWeight: 600, background: activeTab === t.id ? 'var(--accent)' : 'transparent', color: activeTab === t.id ? 'white' : 'var(--text3)', transition: 'all 0.2s', border: 'none', cursor: 'pointer' }}>{t.label}</button>
            ))}
          </div>
            ...
            <div className="fade-in">
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {stats.map(s => {
                  ...
                  return (
                    <button key={s.id} onClick={() => { setSelectedSub(s.id); setGpsStatus(null); setFaceVerified(false) }} style={{ width: '100%', textAlign: 'left', padding: 20, background: liveSlot ? 'rgba(34,211,160,0.06)' : 'var(--surface)', border: `1px solid ${liveSlot ? 'var(--green)' : 'var(--border)'}`, borderRadius: 'var(--radius)', cursor: 'pointer', transition: 'all 0.2s', display: 'flex', alignItems: 'center', gap: 16 }}>
                      <div style={{ width: 56, height: 56, borderRadius: '50%', flexShrink: 0, background: `conic-gradient(${s.pct >= 75 ? '#22d3a0' : '#ff4d6a'} ${s.pct * 3.6}deg, var(--bg2) 0deg)`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
