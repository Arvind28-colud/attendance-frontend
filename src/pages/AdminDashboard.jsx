...
export default function AdminDashboard({ showToast }) {
  ...
  const [ttFilterSem, setTtFilterSem]       = useState('')
  ...
  useEffect(() => {
    if (tab !== 'dashboard') window.history.pushState({ adminTab: tab }, '')
  }, [tab])
  ...
  useEffect(() => {
    if (tab === 'subjects') {
      setSubFilterCourse(''); setSubFilterSem(''); setFilteredSubjects([]); setSubjectsLoaded(false)
    }
    if (tab === 'teachers') {
      setTeacherFilterDept(''); setFilteredTeachers([]); setTeachersLoaded(false)
    }
    if (tab === 'students') {
      setStudentFilterCourse(''); setStudentFilterSem(''); setFilteredStudents([]); setStudentsLoaded(false)
    }
    if (tab === 'timetable') {
      setTtFilterCourse(''); setTtFilterSem(''); setFilteredTT([]); setTtLoaded(false)
    }
  }, [tab])
  ...
  const loadCourseAtt = async (id) => {
    try { const res = await getCourseAttendance(id); setCourseAtt(res.data) }
    catch { showToast('Failed ❌', 'error') }
  }

  // ── Filter handlers ──────────────────────────────────────
  const handleGetSubjects = () => {
    if (!subFilterCourse && !subFilterSem) { showToast('Please select at least Course or Semester ❌', 'error'); return }
    ...
    setFilteredSubjects(filtered); setSubjectsLoaded(true)
    if (filtered.length === 0) showToast('No subjects found ⚠️', 'error')
  }

  const handleGetTeachers = () => {
    if (!teacherFilterDept) { showToast('Please select a Department ❌', 'error'); return }
    const filtered = teachers.filter(t => t.department_id === parseInt(teacherFilterDept))
    setFilteredTeachers(filtered); setTeachersLoaded(true)
    if (filtered.length === 0) showToast('No teachers found in this department ⚠️', 'error')
  }

  const handleGetStudents = () => {
    if (!studentFilterCourse && !studentFilterSem) { showToast('Please select at least Course or Semester ❌', 'error'); return }
    let filtered = students
    if (studentFilterCourse) filtered = filtered.filter(s => s.course_id === parseInt(studentFilterCourse))
    if (studentFilterSem)    filtered = filtered.filter(s => s.semester === parseInt(studentFilterSem))
    setFilteredStudents(filtered); setStudentsLoaded(true)
    if (filtered.length === 0) showToast('No students found ⚠️', 'error')
  }

  const handleGetTimetable = () => {
    if (!ttFilterCourse && !ttFilterSem) { showToast('Please select at least Course or Semester ❌', 'error'); return }
    const subCourseMap = {}; const subSemMap = {}
    subjects.forEach(s => { subCourseMap[s.id] = s.course_id; subSemMap[s.id] = s.semester })
    let filtered = timetable
    if (ttFilterCourse) filtered = filtered.filter(t => subCourseMap[t.subject_id] === parseInt(ttFilterCourse))
    if (ttFilterSem)    filtered = filtered.filter(t => subSemMap[t.subject_id] === parseInt(ttFilterSem))
    setFilteredTT(filtered); setTtLoaded(true)
    if (filtered.length === 0) showToast('No timetable slots found ⚠️', 'error')
  }

  // ── Reusable Filter Panel ────────────────────────────────
  const FilterPanel = ({ children, activeFilters, onClear }) => (
    <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: 16, marginBottom: 20 }}>
      ...
      {activeFilters.filter(Boolean).length > 0 && (
        <div style={{ display: 'flex', gap: 8, marginTop: 12, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          {activeFilters.filter(Boolean).map((f, i) => (
            <span key={i} style={{ fontSize: 11, fontWeight: 600, padding: '3px 10px', borderRadius: 99, background: f.bg || 'rgba(108,99,255,0.15)', color: f.color || 'var(--accent)', border: `1px solid ${f.border || 'rgba(108,99,255,0.3)'}` }}>
              {f.label}
            </span>
          ))}
          <button onClick={onClear} style={{ fontSize: 11, color: 'var(--text3)', background: 'none', border: 'none', cursor: 'pointer', textDecoration: 'underline', padding: 0 }}>
            Clear
          </button>
        </div>
      )}
  ...
  const EmptyFilterState = ({ message = 'Select filters and click Get Details to view data' }) => (
    <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: 40, textAlign: 'center' }}>
      ...
      <div style={{ color: 'var(--text3)', fontSize: 14 }}>{message}</div>
    </div>
  )
  ...
  const handleAddDept = async (e) => {
    e.preventDefault()
    try { await addDepartment({ name: newDept }); showToast('Department added ✅', 'success'); setNewDept(''); loadAll() }
    catch (err) { showToast(err.response?.data?.detail || 'Failed ❌', 'error') }
  }

  const handleAddCourse = async (e) => {
    e.preventDefault()
    try { await addCourse({ name: newCourse.name, department_id: parseInt(newCourse.department_id) }); showToast('Course added ✅', 'success'); setNewCourse({ name: '', department_id: '' }); loadAll() }
    catch (err) { showToast(err.response?.data?.detail || 'Failed ❌', 'error') }
  }
  ...
  const handleAddSubject = async (e) => {
    e.preventDefault()
    try {
      await addSubject({ name: newSubject.name, course_id: parseInt(newSubject.course_id), teacher_id: newSubject.teacher_id ? parseInt(newSubject.teacher_id) : null, total_classes: parseInt(newSubject.total_classes) || 0, semester: parseInt(newSubject.semester) || 1 })
  ...
  const handleAddTimetable = async (e) => {
    e.preventDefault()
    try { await addTimetable({ subject_id: parseInt(newTT.subject_id), day: newTT.day, start_time: newTT.start_time, end_time: newTT.end_time }); showToast('Timetable slot added ✅', 'success'); setNewTT({ subject_id: '', day: 'Monday', start_time: '09:00', end_time: '10:00' }); loadAll() }
    catch (err) { showToast(err.response?.data?.detail || 'Failed ❌', 'error') }
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex' }}>
      ...
      {/* ── DESKTOP SIDEBAR ── */}
      {!isMobile && (
        <aside style={{ width: 220, background: 'var(--surface)', borderRight: '1px solid var(--border)', display: 'flex', flexDirection: 'column', position: 'fixed', top: 0, bottom: 0, left: 0, zIndex: 200, overflowY: 'auto' }}>
          <div style={{ padding: '20px 16px', borderBottom: '1px solid var(--border)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <div style={{ width: 36, height: 36, borderRadius: 10, background: 'var(--yellow)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Shield size={20} color="#000"/></div>
              <div><div style={{ fontWeight: 700, fontSize: 14 }}>Admin</div><div style={{ fontSize: 11, color: 'var(--text3)' }}>{user?.name}</div></div>
            </div>
          </div>
          <nav style={{ padding: '12px 8px', flex: 1 }}>
            {navItems.map(item => (
              <button key={item.id} onClick={() => handleTabChange(item.id)} style={{ width: '100%', textAlign: 'left', padding: '9px 12px', marginBottom: 2, borderRadius: 'var(--radius2)', background: tab === item.id ? 'var(--accent)' : 'transparent', color: tab === item.id ? 'white' : 'var(--text2)', display: 'flex', alignItems: 'center', gap: 10, fontSize: 13, fontWeight: 600, transition: 'all 0.15s' }}>
      ...
      {/* ── MOBILE TOP HEADER ── */}
      {isMobile && (
        <header style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 300, background: 'var(--surface)', borderBottom: '1px solid var(--border)', height: 56, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 16px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ width: 32, height: 32, borderRadius: 8, background: 'var(--yellow)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Shield size={18} color="#000"/></div>
            <div><div style={{ fontWeight: 700, fontSize: 13 }}>Admin</div><div style={{ fontSize: 10, color: 'var(--text3)' }}>{user?.name}</div></div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <button onClick={() => { logout(); navigate('/') }} style={{ background: 'none', border: 'none', color: 'var(--text3)', cursor: 'pointer', padding: 6 }}><LogOut size={18}/></button>
            <button onClick={() => setSidebarOpen(true)} style={{ background: 'none', border: 'none', color: 'var(--text)', cursor: 'pointer', padding: 6 }}><Menu size={22}/></button>
          </div>
        </header>
      )}
        ...
        <>
          ...
          <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: 260, background: 'var(--surface)', zIndex: 500, borderLeft: '1px solid var(--border)', display: 'flex', flexDirection: 'column' }}>
            ...
            <nav style={{ padding: '12px 8px', flex: 1, overflowY: 'auto' }}>
              {navItems.map(item => (
                <button key={item.id} onClick={() => handleTabChange(item.id)} style={{ width: '100%', textAlign: 'left', padding: '12px 14px', marginBottom: 4, borderRadius: 'var(--radius2)', background: tab === item.id ? 'var(--accent)' : 'transparent', color: tab === item.id ? 'white' : 'var(--text2)', display: 'flex', alignItems: 'center', gap: 12, fontSize: 14, fontWeight: 600 }}>
                  {item.icon} {item.label}
                </button>
              ))}
            </nav>
      ...
      {/* ── MAIN CONTENT ── */}
      <main style={{ marginLeft: isMobile ? 0 : 220, flex: 1, padding: isMobile ? '72px 16px 80px' : '28px 24px', overflowY: 'auto', minWidth: 0 }}>
              ...
              <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: 16 }}>
                ...
                {stats && (
                  <ResponsiveContainer width="100%" height={160}>
                    <PieChart>
                      <Pie data={[{ name: 'Students', value: stats.students },{ name: 'Teachers', value: stats.teachers },{ name: 'Subjects', value: stats.subjects },{ name: 'Courses', value: stats.courses }]} cx="50%" cy="50%" innerRadius={40} outerRadius={65} paddingAngle={4} dataKey="value">
              ...
              <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: 16 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginTop: 10 }}>
                  {navItems.slice(1).map(item => (
                    <button key={item.id} onClick={() => handleTabChange(item.id)} style={{ padding: '9px 12px', borderRadius: 'var(--radius2)', background: 'var(--bg2)', border: '1px solid var(--border)', color: 'var(--text2)', display: 'flex', alignItems: 'center', gap: 10, fontSize: 13, fontWeight: 600, textAlign: 'left' }}>
                    ...
                    <tbody>
                      {depts.map(d => (
                        <tr key={d.id}>
                          ...
                          <td><button onClick={async () => { await deleteDepartment(d.id); showToast('Deleted ✅', 'success'); loadAll() }} className="btn btn-danger" style={{ padding: '5px 10px', fontSize: 12 }}><Trash2 size={13}/></button></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                  ...
                  <tbody>
                    {courses.map(c => (
                      <tr key={c.id}>
                        ...
                        <td><button onClick={async () => { await deleteCourse(c.id); showToast('Deleted ✅', 'success'); loadAll() }} className="btn btn-danger" style={{ padding: '5px 10px', fontSize: 12 }}><Trash2 size={13}/></button></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        {tab === 'subjects' && (
          <div className="fade-in">
            ...
            <FilterPanel
              activeFilters={[
                subFilterCourse && { label: courses.find(c => c.id === parseInt(subFilterCourse))?.name },
                subFilterSem    && { label: `Semester ${subFilterSem}`, bg: 'rgba(255,209,102,0.15)', color: 'var(--yellow)', border: 'rgba(255,209,102,0.3)' },
              ]}
              onClear={() => { setSubFilterCourse(''); setSubFilterSem(''); setFilteredSubjects([]); setSubjectsLoaded(false) }}
            >
              <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr auto', gap: 12, alignItems: 'end' }}>
                <div>
                  <label style={{ fontSize: 11, fontWeight: 600, color: 'var(--text3)', display: 'block', marginBottom: 6 }}>Course</label>
                  <select className="select-field" value={subFilterCourse} onChange={e => { setSubFilterCourse(e.target.value); setSubjectsLoaded(false) }}>
                    <option value="">All Courses</option>
                    {courses.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                  </select>
                </div>
                <div>
                  <label style={{ fontSize: 11, fontWeight: 600, color: 'var(--text3)', display: 'block', marginBottom: 6 }}>Semester</label>
                  <select className="select-field" value={subFilterSem} onChange={e => { setSubFilterSem(e.target.value); setSubjectsLoaded(false) }}>
                    <option value="">All Semesters</option>
                    {[1,2,3,4,5,6].map(s => <option key={s} value={s}>Semester {s}</option>)}
                  </select>
                </div>
                <button onClick={handleGetSubjects} className="btn btn-primary" style={{ justifyContent: 'center', padding: '10px 20px', whiteSpace: 'nowrap' }}>
                  <Search size={15}/> Get Details
                </button>
              </div>
            </FilterPanel>

            {!subjectsLoaded ? <EmptyFilterState/> : filteredSubjects.length === 0 ? (
                ...
                <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', overflow: 'hidden' }}>
                  <div style={{ overflowX: 'auto' }}>
                    <table style={{ minWidth: 500 }}>
                      <thead><tr><th>Subject</th><th>Course</th><th>Teacher</th><th>Sem</th><th>Classes</th><th></th></tr></thead>
                      <tbody>
                        {filteredSubjects.map(s => (
                          <tr key={s.id}>
                            ...
                            <td>
                              <button onClick={async () => { await deleteSubject(s.id); showToast('Deleted ✅', 'success'); await loadAll(); setTimeout(() => handleGetSubjects(), 300) }} className="btn btn-danger" style={{ padding: '5px 10px', fontSize: 12 }}><Trash2 size={13}/></button>
                            </td>
                          </tr>
                        ))}
                      </tbody>
        ...
        {tab === 'teachers' && (
          <div className="fade-in">
            ...
            <FilterPanel
              activeFilters={[
                teacherFilterDept && { label: depts.find(d => d.id === parseInt(teacherFilterDept))?.name },
              ]}
              onClear={() => { setTeacherFilterDept(''); setFilteredTeachers([]); setTeachersLoaded(false) }}
            >
              <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr auto', gap: 12, alignItems: 'end' }}>
                <div>
                  <label style={{ fontSize: 11, fontWeight: 600, color: 'var(--text3)', display: 'block', marginBottom: 6 }}>Department</label>
                  <select className="select-field" value={teacherFilterDept} onChange={e => { setTeacherFilterDept(e.target.value); setTeachersLoaded(false) }}>
                    <option value="">Select Department</option>
                    {depts.map(d => <option key={d.id} value={d.id}>{d.name}</option>)}
                  </select>
                </div>
                <button onClick={handleGetTeachers} className="btn btn-primary" style={{ justifyContent: 'center', padding: '10px 20px', whiteSpace: 'nowrap' }}>
                  <Search size={15}/> Get Details
                </button>
              </div>
            </FilterPanel>
            ...
            {!teachersLoaded ? (
              <EmptyFilterState message="Select a department and click Get Details to view teachers"/>
            ) : filteredTeachers.length === 0 ? (
              <div className="empty-state">No teachers found in this department</div>
            ) : (
              <div className="fade-in">
                <div style={{ fontSize: 13, color: 'var(--text3)', marginBottom: 12 }}>
                  Showing <strong style={{ color: 'var(--text)' }}>{filteredTeachers.length}</strong> teacher{filteredTeachers.length !== 1 ? 's' : ''}
                  {teacherFilterDept && ` in ${depts.find(d => d.id === parseInt(teacherFilterDept))?.name}`}
                </div>
                <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', overflow: 'hidden' }}>
                  <div style={{ overflowX: 'auto' }}>
                      ...
                      <tbody>
                        {filteredTeachers.map(t => (
                          <tr key={t.id}>
                            ...
                            <td><button onClick={() => setConfirmModal({ type: 'teacher', id: t.id, name: t.name })} className="btn btn-danger" style={{ padding: '5px 10px', fontSize: 12 }}><Trash2 size={13}/></button></td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
        ...
        {/* ── STUDENTS — filter by Course + Semester ── */}
        {tab === 'students' && (
          <div className="fade-in">
            ...
            <FilterPanel
              activeFilters={[
                studentFilterCourse && { label: courses.find(c => c.id === parseInt(studentFilterCourse))?.name },
                studentFilterSem    && { label: `Semester ${studentFilterSem}`, bg: 'rgba(255,209,102,0.15)', color: 'var(--yellow)', border: 'rgba(255,209,102,0.3)' },
              ]}
              onClear={() => { setStudentFilterCourse(''); setStudentFilterSem(''); setFilteredStudents([]); setStudentsLoaded(false) }}
            >
              <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr auto', gap: 12, alignItems: 'end' }}>
                <div>
                  <label style={{ fontSize: 11, fontWeight: 600, color: 'var(--text3)', display: 'block', marginBottom: 6 }}>Course</label>
                  <select className="select-field" value={studentFilterCourse} onChange={e => { setStudentFilterCourse(e.target.value); setStudentsLoaded(false) }}>
                    <option value="">All Courses</option>
                    {courses.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                  </select>
                </div>
                <div>
                  <label style={{ fontSize: 11, fontWeight: 600, color: 'var(--text3)', display: 'block', marginBottom: 6 }}>Semester</label>
                  <select className="select-field" value={studentFilterSem} onChange={e => { setStudentFilterSem(e.target.value); setStudentsLoaded(false) }}>
                    <option value="">All Semesters</option>
                    {[1,2,3,4,5,6].map(s => <option key={s} value={s}>Semester {s}</option>)}
                  </select>
                </div>
                <button onClick={handleGetStudents} className="btn btn-primary" style={{ justifyContent: 'center', padding: '10px 20px', whiteSpace: 'nowrap' }}>
                  <Search size={15}/> Get Details
                </button>
              </div>
            </FilterPanel>
            ...
            {!studentsLoaded ? (
              <EmptyFilterState message="Select filters and click Get Details to view students"/>
            ) : filteredStudents.length === 0 ? (
              <div className="empty-state">No students found for selected filters</div>
            ) : (
              <div className="fade-in">
                <div style={{ fontSize: 13, color: 'var(--text3)', marginBottom: 12 }}>
                  Showing <strong style={{ color: 'var(--text)' }}>{filteredStudents.length}</strong> student{filteredStudents.length !== 1 ? 's' : ''}
                  {studentFilterCourse && ` in ${courses.find(c => c.id === parseInt(studentFilterCourse))?.name}`}
                  {studentFilterSem && ` · Semester ${studentFilterSem}`}
                </div>
                <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', overflow: 'hidden' }}>
                  <div style={{ overflowX: 'auto' }}>
                      ...
                      <tbody>
                        {filteredStudents.map(s => (
                          <tr key={s.id}>
                            ...
                            <td>{s.course_name}</td>
                            <td><span className="badge badge-yellow">Sem {s.semester}</span></td>
                            <td><button onClick={() => setConfirmModal({ type: 'student', id: s.id, name: s.name })} className="btn btn-danger" style={{ padding: '5px 10px', fontSize: 12 }}><Trash2 size={13}/></button></td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
        ...
        {/* ── TIMETABLE — filter by Course + Semester ── */}
        {tab === 'timetable' && (() => {
          const subCourseMap = {}; const subSemMap = {}
          subjects.forEach(s => { subCourseMap[s.id] = s.course_id; subSemMap[s.id] = s.semester })
          const addableSubjects = ttFilterCourse ? subjects.filter(s => s.course_id === parseInt(ttFilterCourse)) : subjects
          const byDay = {}
          ...
          return (
            <div className="fade-in">
              ...
              <p style={{ color: 'var(--text3)', fontSize: 13, marginBottom: 16 }}>Filter by course and semester to view slots.</p>
              ...
              <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: 16, marginBottom: 20 }}>
                ...
                <form onSubmit={handleAddTimetable} style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : 'repeat(auto-fit, minmax(140px, 1fr))', gap: 10 }}>
                  <div style={{ gridColumn: isMobile ? 'span 2' : 'auto' }}>
                    ...
                    <select className="select-field" value={ttFilterCourse} onChange={e => { setTtFilterCourse(e.target.value); setNewTT(f => ({ ...f, subject_id: '' })); setTtLoaded(false) }}>
                      <option value="">All courses</option>
                      {courses.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                    </select>
                  </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-end' }}>
                    <button type="submit" className="btn btn-primary" style={{ width: '100%', justifyContent: 'center' }}><Plus size={15}/> Add</button>
                  </div>
                </form>
              </div>
              ...
              <FilterPanel
                activeFilters={[
                  ttFilterCourse && { label: courses.find(c => c.id === parseInt(ttFilterCourse))?.name },
                  ttFilterSem    && { label: `Semester ${ttFilterSem}`, bg: 'rgba(255,209,102,0.15)', color: 'var(--yellow)', border: 'rgba(255,209,102,0.3)' },
                ]}
                onClear={() => { setTtFilterCourse(''); setTtFilterSem(''); setFilteredTT([]); setTtLoaded(false) }}
              >
                <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr auto', gap: 12, alignItems: 'end' }}>
                  <div>
                    <label style={{ fontSize: 11, fontWeight: 600, color: 'var(--text3)', display: 'block', marginBottom: 6 }}>Course</label>
                    <select className="select-field" value={ttFilterCourse} onChange={e => { setTtFilterCourse(e.target.value); setTtLoaded(false); setNewTT(f => ({ ...f, subject_id: '' })) }}>
                      <option value="">All Courses</option>
                      {courses.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                    </select>
                  </div>
                  <div>
                    <label style={{ fontSize: 11, fontWeight: 600, color: 'var(--text3)', display: 'block', marginBottom: 6 }}>Semester</label>
                    <select className="select-field" value={ttFilterSem} onChange={e => { setTtFilterSem(e.target.value); setTtLoaded(false) }}>
                      <option value="">All Semesters</option>
                      {[1,2,3,4,5,6].map(s => <option key={s} value={s}>Semester {s}</option>)}
                    </select>
                  </div>
                  <button onClick={handleGetTimetable} className="btn btn-primary" style={{ justifyContent: 'center', padding: '10px 20px', whiteSpace: 'nowrap' }}>
                    <Search size={15}/> Get Details
                  </button>
                </div>
              </FilterPanel>
              ...
              {/* Results */}
              {!ttLoaded ? (
                <EmptyFilterState message="Select filters and click Get Details to view timetable"/>
              ) : filteredTT.length === 0 ? (
                <div className="empty-state card">No timetable slots found for selected filters.</div>
              ) : (
                <div className="fade-in">
                  <div style={{ fontSize: 13, color: 'var(--text3)', marginBottom: 12 }}>
                    Showing <strong style={{ color: 'var(--text)' }}>{filteredTT.length}</strong> slot{filteredTT.length !== 1 ? 's' : ''}
                    {ttFilterCourse && ` in ${courses.find(c => c.id === parseInt(ttFilterCourse))?.name}`}
                    {ttFilterSem && ` · Semester ${ttFilterSem}`}
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                    {DAYS.filter(d => byDay[d].length > 0).map(day => (
                      <div key={day} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', overflow: 'hidden' }}>
                        ...
                        <div style={{ overflowX: 'auto' }}>
                          <table style={{ minWidth: 380 }}>
                            <thead><tr><th>Subject</th><th>Course</th><th>Sem</th><th>Time</th><th></th></tr></thead>
                            <tbody>
                              {byDay[day].map(t => {
                                const course = courses.find(c => c.id === subCourseMap[t.subject_id])
                                const sem = subSemMap[t.subject_id]
                                return (
                                  <tr key={t.id}>
                                    <td style={{ fontWeight: 600, color: 'var(--text)' }}>{t.subject_name}</td>
                                    <td>{course ? <span className="badge badge-purple">{course.name}</span> : '—'}</td>
                                    <td>{sem ? <span className="badge badge-yellow">Sem {sem}</span> : '—'}</td>
                                    <td style={{ fontFamily: 'Space Mono', fontSize: 12, whiteSpace: 'nowrap' }}>{t.start_time}–{t.end_time}</td>
                                    <td>
                                      <button onClick={async () => { await deleteTimetable(t.id); showToast('Deleted ✅', 'success'); await loadAll(); setTimeout(() => handleGetTimetable(), 300) }} className="btn btn-danger" style={{ padding: '5px 10px', fontSize: 12 }}><Trash2 size={13}/></button>
                                    </td>
                                  </tr>
                                )
                              })}
      ...
      {isMobile && (
        <nav style={{ position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 300, background: 'var(--surface)', borderTop: '1px solid var(--border)', display: 'flex', height: 60 }}>
          {bottomNavItems.map(item => (
            <button key={item.id} onClick={() => handleTabChange(item.id)} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 3, border: 'none', background: 'none', cursor: 'pointer', color: tab === item.id ? 'var(--accent)' : 'var(--text3)', fontSize: 10, fontWeight: 600, borderTop: tab === item.id ? '2px solid var(--accent)' : '2px solid transparent' }}>
              {item.icon}<span>{item.label}</span>
            </button>
          ))}
          <button onClick={() => setSidebarOpen(true)} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 3, border: 'none', background: 'none', cursor: 'pointer', color: drawerItems.some(i => i.id === tab) ? 'var(--accent)' : 'var(--text3)', fontSize: 10, fontWeight: 600, borderTop: drawerItems.some(i => i.id === tab) ? '2px solid var(--accent)' : '2px solid transparent' }}>
